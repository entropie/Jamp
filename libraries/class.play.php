	...
	class play extends main {
		...
		function playsongs($ids) {
			if(!is_array($ids))
				...
				# this was an evil bug!
				# the root problem will be find, sometime
		...
		function extractids ($plids, $xml = false) {
			...
			foreach($plids as $plid) {
				...
				$path = $this->checkfileSlash($path["path"]);
				...
				while($row = mysql_fetch_array($result)) {
					if(!empty($row["id"])) {
						if($xml && !empty($row["path"]))
							$ids[] = array("id" => $row["id"], "path" => $row["path"], "file" => $row["file"], "prim_path" => $row["prim_path"], "pid" => $row["pid"],"prim_pid" => $row["ppid"]);
						else
							$ids[] = $row["id"];
					}
				}
